...
public final class DuelManager {
  ...
  /**
   * Initializes a DuelManager and registers it as the singleton instance returned
   * by getInstance().
   */
  public DuelManager() {
    instance = this;
  ...
  /**
   * Accepts a pending duel request for the given player and initiates the duel on
   * an available duel service.
   * <p>
   * </p>
   * If the player has no pending request, the player is notified and no further
   * action is taken. If the resolved
   * duel service reports a status other than AVAILABLE, the player is notified
   * and a {@link ServiceUnavailabException}
   * is thrown.
   *
   * @param networkPlayer the player attempting to accept a duel request
   * @throws ServiceUnavailableException if the selected duel service reports a
  ...
  /**
   * Creates and stores a duel request for the receiver and notifies them with a
   * clickable, hoverable acceptance message.
   *
   * <p>
   * The request is cached on the receiver for 120 seconds. The notification
   * contains a click action to accept the duel
  ...
  public void requestDuel(
    ...
    receiver.whereIs().thenAccept(clusterService -> {
      ...
      receiver.sendMessage(
          "<#3B82F6><player> <white>sent you a duel request! Accept the request by <click:run_command:'/duel accept'><hover:show_text:'<#3B82F6>Click to Accept'><#3B82F6>[Clicking Here]</hover></click>. You can view the duel settings by <hover:show_text:'<#3B82F6>Rounds: <rounds>\n<#3B82F6>Kit: <kit>'><#3B82F6>[Hovering Here]</hover>."
  ...
  /**
   * Adds the given player to the matchmaking queue for the specified kit and
   * queue type.
   *
