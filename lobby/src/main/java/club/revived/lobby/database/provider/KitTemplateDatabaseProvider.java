...
public final class KitTemplateDatabaseProvider implements DatabaseProvider<KitTemplate> {
  ...
  /**
   * Creates a KitDatabaseProvider and ensures the MongoDB collection named "kits"
   * exists.
   *
   * @param database the MongoDatabase used to create and access the "kits"
   *                 collection
   */
   ...
   *
   * This creates an index to improve lookup performance for operations that query
   * kits by UUID.
   */
  @Override
  public void start() {
   ...
   * @throws RuntimeException if an error occurs while serializing or writing to
   *                          the database
   */
  @Override
  public void save(final KitTemplate kitHolder) {
    try {
      ...
      System.out.println("Savin KitTemplate");
   ...
   * @return an Optional containing the deserialized KitHolder if found, otherwise
   *         Optional.empty()
