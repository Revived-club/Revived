...
public final class PlayerManager {
  ...
  /**
   * Creates a PlayerManager, assigns this object as the singleton instance, and
   * registers message handlers for inter-server messaging.
   */
   ...
   * @param currentServer the name of the server the player is currently connected
   *                      to
   */
  ...
  /**
   * Adds the given NetworkPlayer to the manager's registry keyed by the player's
   * UUID.
   *
   * @param networkPlayer the NetworkPlayer to register; its UUID is used as the
   *                      registry key
   */
  ...
  /**
   * Installs cluster message handlers that deliver inter-server chat payloads to
   * Bukkit players.
   *
   * <p>
   * Registers handlers for:
   * <ul>
   * <li>`SendMessage` — resolves a target by UUID and delivers a rich chat
   * message to that player.</li>
   * <li>`BroadcastMessage` — delivers a rich chat message to every online
   * player.</li>
   * <li>`SendActionbar` — resolves a target by UUID and delivers a parsed action
   * bar message to that player.</li>
   * </ul>
   */
   ...
   * @throws UnregisteredPlayerException if no NetworkPlayer is registered for the
   *                                     player's UUID
   */
  @NotNull
   ...
   * @return the NetworkPlayer mapped to the UUID, or {@code null} if no mapping
   *         exists
   * @throws UnregisteredPlayerException if the internal registry already contains
   *                                     the given UUID
   */
  @NotNull
   ...
   * @return the first matching NetworkPlayer, or `null` if no registered player
   *         matches
   */
  @Nullable
   ...
   *
   * @return the live map from UUID to NetworkPlayer; modifying the returned map
   *         updates this manager's registry
   */
